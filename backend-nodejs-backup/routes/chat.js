        ...
        const conversas = db.prepare(`
            ...
            SELECT 
                ...
                (SELECT mensagem FROM mensagens 
                 WHERE (remetente_id = ? AND destinatario_id = c.contato_id) 
                    OR (remetente_id = c.contato_id AND destinatario_id = ?)
                ...
                (SELECT COUNT(*) FROM mensagens 
                 WHERE remetente_id = c.contato_id 
                   AND destinatario_id = ? 
                   AND lido = 0) as nao_lidas
            FROM contatos c
            JOIN usuarios u ON u.id = c.contato_id
