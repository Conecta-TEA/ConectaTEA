...
document.getElementById('formEmail').addEventListener('submit', async (e) => {
    ...
    console.log('[LOG] Email digitado:', email);
    ...
    try {
        ...
        if (data.sucesso) {
            ...
            console.log('[LOG] Trocando para etapa 2...');
            
            // Trocar para etapa 2 (digitar código)
            formSolicitarCodigo.classList.remove('active');
            formDigitarCodigo.classList.add('active');
            
            console.log('[LOG] Classes atualizadas. formDigitarCodigo ativo:', formDigitarCodigo.classList.contains('active'));
            
            mostrarAlerta('Código enviado para seu email!', 'success');
        } else {
            mostrarAlerta(data.mensagem || 'Erro ao enviar código', 'error');
            console.log('[LOG] Erro ao enviar código:', data.mensagem);
        }

    } catch (error) {
        mostrarLoading(false);
        console.error('[LOG] Erro no fetch:', error);
