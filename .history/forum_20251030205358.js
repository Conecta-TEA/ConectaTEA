...
function renderizarPosts(posts) {
    ...
    if (!posts || posts.length === 0) {
        container.innerHTML = '<div class="empty-state"><i class="fas fa-comments"></i><p>Nenhuma discussão encontrada. Seja o primeiro a criar!</p></div>';
        return;
    }
...
function renderizarPopulares(posts) {
    ...
    container.innerHTML = posts.map(p => `
        <div class="popular-post-item" onclick="verPost(${p.id})">
            <h4>${p.titulo}</h4>
            <p>${p.visualizacoes || 0} visualizações</p>
        </div>
    `).join('');
}
